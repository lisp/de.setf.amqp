                 ...
                 ,@(let ((type-spec (table-type-spec)))
                     (when type-spec
                       `((defun ,buffer-table-op (buffer position)
                         ...
                         (defun (setf ,buffer-table-op) (table buffer position &optional type-code-p)
                 ...
                 ,@(let ((type-spec (list-type-spec)))
                     (when type-spec
                       `((defun ,buffer-list-op (buffer position)
                         ...
                         (defun (setf ,buffer-list-op) (list buffer position &optional (type-code-p nil))
                           (optionally-set-type ,(getf type-spec :line-code))
                           (let ((base position))
                             (incf position 4)
                             (dolist (value list)
                           ...
                           (let* ((length (buffer-unsigned-byte-32 buffer position))
                             ...
                             (loop (when (>= position end)
                                   ...
                                   (multiple-value-bind (value new-position)
                                     ...
                                     (setf position new-position)))
                             (values result end)))
                         (defun (setf ,buffer-array-op) (array buffer position &optional (type-code-p nil))
                           (optionally-set-type ,(getf type-spec :line-code))
                           (let ((base position)
